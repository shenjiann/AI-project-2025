...
_preprocess = transforms.Compose([
    transforms.Resize((227, 227)),   # 和训练保持一致
    transforms.ToTensor(),
...
# 把字符串层名映射到真正的子模块
def _get_layer(model, which: str):
    # 按你的模型结构改这里：
    # 例：model.features[0], [3], [6], [8], [10] 分别为 5 个 conv
    mapping = {
        "conv1": model.features[0],
...
@torch.no_grad()
def get_feature_maps(which: str, img_path) -> np.ndarray:
    layer = _get_layer(model, which)
    feats = {}
    ...
    try:
        ...
        x = _preprocess(img).unsqueeze(0).to(next(model.parameters()).device)
        model.eval()
        _ = model(x)
        arr = feats["x"].squeeze(0).numpy()  # [C, H, W]
        return arr
    finally:
        h.remove()