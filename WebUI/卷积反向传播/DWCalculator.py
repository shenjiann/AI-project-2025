...
class DWCalculator():
    def __init__(self, data_instance: Data):
        ...
        self._dW_cache = reactive.Value(
            torch.zeros_like(self.data.W, dtype=torch.float32))
        self._dW_cache_last = reactive.Value(
            torch.zeros_like(self.data.W, dtype=torch.float32))
    ...
    @property
    def dW_cache(self):
        return self._dW_cache.get()

    @property
    def dW_cache_last(self):
        return self._dW_cache_last.get()
    ...
    def get_dW_slice_ij(self):
        return self.get_dZ0_ij() * self.get_Z_slice_ij()

    def add_to_dW_cache(self, dW_slice_val):
        self._dW_cache_last.set(self._dW_cache.get().clone())
        coords = self.get_focus_ij()
        if not coords:
            return None
        c, i, j = coords[0]
        current_dW_cache = self._dW_cache.get().clone()
        current_dW_cache += dW_slice_val
        self._dW_cache.set(current_dW_cache)

        # 如果当前步数是最后一个有效步数，则设置标志位
        if self.steps == self.data._elems_per_chan:
            self._last_slice_accumulated.set(True)

    def reset_dW_cache(self):
        self._dW_cache.set(
            torch.zeros_like(self.data.W, dtype=torch.float32))
